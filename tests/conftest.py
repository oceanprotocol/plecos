...
def schema_local_dict():
    return _load_sample_path(plecos.LOCAL_SCHEMA_FILE, f'Loaded schema: {plecos.LOCAL_SCHEMA_FILE}')


@pytest.fixture
def schema_remote_dict():
    return _load_sample_path(plecos.REMOTE_SCHEMA_FILE, f'Loaded schema: {plecos.REMOTE_SCHEMA_FILE}')


@pytest.fixture
def sample_metadata_dict_local():
    return _load_sample_path(PATH_SAMPLE_METADATA_LOCAL, f'Loaded sample: {PATH_SAMPLE_METADATA_LOCAL}')


@pytest.fixture
def sample_metadata_dict_remote():
    return _load_sample_path(PATH_SAMPLE_METADATA_REMOTE, f'Loaded sample: {PATH_SAMPLE_METADATA_REMOTE}')
...
def sample_algorithm_md_dict_local():
    return _load_sample_path(PATH_ALGORITHM_METADATA_LOCAL, f'Loaded sample: {PATH_ALGORITHM_METADATA_LOCAL}')


@pytest.fixture
def sample_algorithm_md_dict_remote():
    return _load_sample_path(PATH_ALGORITHM_METADATA_REMOTE, f'Loaded sample: {PATH_ALGORITHM_METADATA_REMOTE}')
